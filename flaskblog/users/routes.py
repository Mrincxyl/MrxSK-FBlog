...
users = Blueprint('users',__name__) # 'users' -> name of our blueprint

# whatever the variable name is will be used for blueprint
@users.route("/register",methods=['GET','POST']) #-> for new users
def register():
...
@users.route("/login",methods=['GET','POST']) #-> already registered users
def login():
    if current_user.is_authenticated:
